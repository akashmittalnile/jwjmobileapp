  ...
  const submitHandler = async (values: any) => {
    try {
      ...
      if (!data?.file) {
        ...
        //   const imageName = profile.path.slice(
        //     profile.path.lastIndexOf('/'),
        //     profile.path.length,
        //   );
      ...
      if (data?.mobile?.replace(/\D/g, '')?.length !== 10 || !data1?.name) {
        if (data1?.mobile?.length !== 10) {
          setErr(preData => ({...preData, mobile: true}));
        }
        if (!data1?.name) {
      ...
      <KeyboardAvoidingView
        ...
        <Formik
          ...
          {({
            ...
            <>
              <TextInputField
                containerStyle={{...styles.input, marginTop: responsiveHeight(2)}}
                uri={userIconPath}
                placeholder="Name"
                onChangeText={text => {
                  handleChange('name')(text);
